/**
 * Columns the app expects on public.profile_settings.
 * Keep in sync with supabase/v2_features.sql + supabase/v3_features.sql.
 */
export const REQUIRED_PROFILE_SETTINGS_COLUMNS = [
  "profile_id",
  "layout",
  "accent_color",
  "text_color",
  "background_color",
  "font_family",
  "animated_gradient",
  "gradient_colors",
  "glassmorphism",
  "neon_glow",
  "border_radius",
  "profile_opacity",
  "profile_blur",
  "background_type",
  "background_image_url",
  "background_video_url",
  "particle_effect",
  "overlay_opacity",
  "vignette",
  "noise_texture",
  "music_url",
  "music_title",
  "music_autoplay",
  "music_loop",
  "music_volume",
  "music_show_player",
  "cursor_effect",
  "cursor_image_url",
  "cursor_image_size",
  "cursor_hotspot_x",
  "cursor_hotspot_y",
  "profile_favicon_url",
  "tab_title_animation",
  "typing_bio",
  "bio_color",
  "bio_font_family",
  "bio_font_size",
  "bio_font_weight",
  "bio_italic",
  "bio_glow",
  "bio_letter_spacing",
  "username_effect",
  "hover_animations",
  "page_entrance_animation",
  "link_animation",
  "show_view_count",
  "show_join_date",
  "profile_status",
  "profile_status_color",
  "music_player_color",
  "featured_link_id",
  "show_badges",
  "badge_display_limit",
  "badges_monochrome",
  "badges_custom_monochrome",
  "badges_glow",
  "badge_color",
  "links_monochrome",
  "links_style",
  "links_icon_size",
  "links_icon_glow",
  "links_icon_shadow",
  "links_icon_pulse",
  "links_spacing",
  "links_button_style",
  "links_border_radius",
  "links_button_opacity",
  "links_show_hostname",
  "show_total_followers",
  "profile_parallax",
  "content_alignment",
  "page_nav_position",
  "status_preset",
  "status_emoji",
  "guestbook_enabled",
  "guestbook_approval_required",
  "guestbook_use_profile_card",
  "guestbook_opacity",
  "guestbook_blur",
  "guestbook_glassmorphism",
  "guestbook_show_background",
  "guestbook_background_color",
  "guestbook_message_opacity",
  "guestbook_author_opacity",
  "guestbook_label_opacity",
  "guestbook_text_color",
  "guestbook_border_style",
  "guestbook_spacing",
  "guestbook_padding_y",
  "show_follow_counts",
  "show_activity",
  "friends_visibility",
  "enter_gate_enabled",
  "enter_gate_title",
  "enter_gate_subtitle",
  "enter_gate_button",
  "enter_gate_show_avatar",
  "enter_gate_blur",
  "enter_gate_blur_strength",
  "enter_gate_background_type",
  "enter_gate_background_color",
  "enter_gate_background_image_url",
  "enter_gate_background_video_url",
  "enter_gate_gradient_colors",
  "enter_gate_animated_gradient",
  "enter_gate_overlay_opacity",
  "enter_gate_vignette",
  "enter_gate_noise",
  "enter_gate_particle_effect",
  "enter_gate_show_username",
  "enter_gate_show_branding",
  "enter_gate_title_color",
  "enter_gate_subtitle_color",
  "enter_gate_accent_color",
  "enter_gate_text_align",
  "enter_gate_button_style",
  "enter_gate_animation",
  "enter_gate_glass_card",
  "enter_gate_card_opacity",
  "enter_gate_font_family",
  "layout_label",
  "layout_primary_color",
  "layout_secondary_color",
  "layout_tertiary_color",
  "layout_hide_border",
  "hide_card_border",
  "card_border_effect",
  "card_border_thickness",
  "card_border_speed",
  "card_border_glow_intensity",
  "card_border_color",
  "card_border_secondary_color",
  "card_border_apply_all",
  "card_border_targets",
  "profile_avatar_effect",
  "profile_avatar_effect_thickness",
  "profile_avatar_effect_speed",
  "profile_avatar_effect_glow",
  "profile_avatar_effect_color",
  "profile_avatar_effect_secondary_color",
  "card_offset_x",
  "card_offset_y",
  "card_width",
  "card_max_height",
  "created_at",
  "updated_at",
] as const;

export type SchemaValidationResult =
  | { ok: true }
  | { ok: false; missing: string[]; message: string };

export const SCHEMA_MIGRATION_HINT =
  "Run the relevant supabase/*.sql migrations in the Supabase SQL Editor, then restart the dev server.";

const COLUMN_MIGRATIONS: Record<string, string> = {
  music_title: "supabase/v4_music_title.sql",
  show_badges: "supabase/v5_badges.sql",
  badge_display_limit: "supabase/v5_badges.sql",
  badges_monochrome: "supabase/v7_badge_customization.sql",
  badges_custom_monochrome: "supabase/v57_badges_custom_monochrome.sql",
  badges_glow: "supabase/v31_badges_glow.sql",
  badge_color: "supabase/v7_badge_customization.sql",
  links_monochrome: "supabase/v10_links_parallax.sql",
  links_style: "supabase/v14_links_style.sql",
  links_icon_size: "supabase/v42_links_icon_size.sql",
  links_icon_glow: "supabase/v43_links_icon_effects.sql",
  links_icon_shadow: "supabase/v43_links_icon_effects.sql",
  links_icon_pulse: "supabase/v43_links_icon_effects.sql",
  links_spacing: "supabase/v83_links_customization.sql",
  links_button_style: "supabase/v83_links_customization.sql",
  links_border_radius: "supabase/v83_links_customization.sql",
  links_button_opacity: "supabase/v83_links_customization.sql",
  links_show_hostname: "supabase/v83_links_customization.sql",
  show_total_followers: "supabase/v106_total_followers.sql",
  profile_parallax: "supabase/v10_links_parallax.sql",
  content_alignment: "supabase/v17_content_alignment.sql",
  page_nav_position: "supabase/v84_page_nav_position.sql",
  music_show_player: "supabase/v85_music_show_player.sql",
  profile_status_color: "supabase/v11_ui_colors.sql",
  music_player_color: "supabase/v11_ui_colors.sql",
  status_preset: "supabase/v12_v2_expansion.sql",
  status_emoji: "supabase/v12_v2_expansion.sql",
  guestbook_enabled: "supabase/v12_v2_expansion.sql",
  guestbook_approval_required: "supabase/v12_v2_expansion.sql",
  guestbook_use_profile_card: "supabase/v71_guestbook_appearance.sql",
  guestbook_opacity: "supabase/v71_guestbook_appearance.sql",
  guestbook_blur: "supabase/v71_guestbook_appearance.sql",
  guestbook_glassmorphism: "supabase/v71_guestbook_appearance.sql",
  guestbook_show_background: "supabase/v71_guestbook_appearance.sql",
  guestbook_background_color: "supabase/v71_guestbook_appearance.sql",
  guestbook_message_opacity: "supabase/v71_guestbook_appearance.sql",
  guestbook_author_opacity: "supabase/v71_guestbook_appearance.sql",
  guestbook_label_opacity: "supabase/v71_guestbook_appearance.sql",
  guestbook_text_color: "supabase/v71_guestbook_appearance.sql",
  guestbook_border_style: "supabase/v71_guestbook_appearance.sql",
  guestbook_spacing: "supabase/v71_guestbook_appearance.sql",
  guestbook_padding_y: "supabase/v71_guestbook_appearance.sql",
  show_follow_counts: "supabase/v13_show_follow_counts.sql",
  show_activity: "supabase/v15_show_activity.sql",
  friends_visibility: "supabase/v12_v2_expansion.sql",
  enter_gate_enabled: "supabase/v24_enter_gate.sql",
  enter_gate_title: "supabase/v24_enter_gate.sql",
  enter_gate_subtitle: "supabase/v24_enter_gate.sql",
  enter_gate_button: "supabase/v24_enter_gate.sql",
  enter_gate_show_avatar: "supabase/v24_enter_gate.sql",
  enter_gate_blur: "supabase/v24_enter_gate.sql",
  enter_gate_blur_strength: "supabase/v40_enter_gate_customization.sql",
  enter_gate_background_type: "supabase/v40_enter_gate_customization.sql",
  enter_gate_background_color: "supabase/v40_enter_gate_customization.sql",
  enter_gate_background_image_url: "supabase/v40_enter_gate_customization.sql",
  enter_gate_background_video_url: "supabase/v40_enter_gate_customization.sql",
  enter_gate_gradient_colors: "supabase/v40_enter_gate_customization.sql",
  enter_gate_animated_gradient: "supabase/v40_enter_gate_customization.sql",
  enter_gate_overlay_opacity: "supabase/v40_enter_gate_customization.sql",
  enter_gate_vignette: "supabase/v40_enter_gate_customization.sql",
  enter_gate_noise: "supabase/v40_enter_gate_customization.sql",
  enter_gate_particle_effect: "supabase/v40_enter_gate_customization.sql",
  enter_gate_show_username: "supabase/v40_enter_gate_customization.sql",
  enter_gate_show_branding: "supabase/v40_enter_gate_customization.sql",
  enter_gate_title_color: "supabase/v40_enter_gate_customization.sql",
  enter_gate_subtitle_color: "supabase/v40_enter_gate_customization.sql",
  enter_gate_accent_color: "supabase/v40_enter_gate_customization.sql",
  enter_gate_text_align: "supabase/v40_enter_gate_customization.sql",
  enter_gate_button_style: "supabase/v40_enter_gate_customization.sql",
  enter_gate_animation: "supabase/v40_enter_gate_customization.sql",
  enter_gate_glass_card: "supabase/v40_enter_gate_customization.sql",
  enter_gate_card_opacity: "supabase/v40_enter_gate_customization.sql",
  enter_gate_font_family: "supabase/v73_enter_gate_font.sql",
  layout_label: "supabase/v27_layout_label.sql",
  layout_primary_color: "supabase/v102_layout_colors.sql",
  layout_secondary_color: "supabase/v102_layout_colors.sql",
  layout_tertiary_color: "supabase/v102_layout_colors.sql",
  layout_hide_border: "supabase/v103_layout_hide_border.sql",
  hide_card_border: "supabase/v28_card_layout.sql",
  card_border_effect: "supabase/v70_card_border_effects.sql",
  card_border_thickness: "supabase/v70_card_border_effects.sql",
  card_border_speed: "supabase/v70_card_border_effects.sql",
  card_border_glow_intensity: "supabase/v70_card_border_effects.sql",
  card_border_color: "supabase/v70_card_border_effects.sql",
  card_border_secondary_color: "supabase/v70_card_border_effects.sql",
  card_border_apply_all: "supabase/v70_card_border_effects.sql",
  card_border_targets: "supabase/v70_card_border_effects.sql",
  profile_avatar_effect: "supabase/v108_profile_avatar_effects.sql",
  profile_avatar_effect_thickness: "supabase/v108_profile_avatar_effects.sql",
  profile_avatar_effect_speed: "supabase/v108_profile_avatar_effects.sql",
  profile_avatar_effect_glow: "supabase/v108_profile_avatar_effects.sql",
  profile_avatar_effect_color: "supabase/v108_profile_avatar_effects.sql",
  profile_avatar_effect_secondary_color: "supabase/v108_profile_avatar_effects.sql",
  card_offset_x: "supabase/v28_card_layout.sql",
  card_offset_y: "supabase/v28_card_layout.sql",
  card_width: "supabase/v28_card_layout.sql",
  card_max_height: "supabase/v48_card_max_height.sql",
  show_discord_status: "supabase/v34_discord_connect.sql",
  discord_username: "supabase/v34_discord_connect.sql",
  discord_avatar: "supabase/v34_discord_connect.sql",
  discord_banner: "supabase/v63_discord_premium_type.sql",
  discord_premium_type: "supabase/v63_discord_premium_type.sql",
  custom_theme_id: "supabase/v35_custom_themes.sql",
  active_preset_id: "supabase/v64_profile_presets.sql",
  overlay_opacity: "supabase/v3_features.sql",
  vignette: "supabase/v3_features.sql",
  noise_texture: "supabase/v3_features.sql",
  cursor_effect: "supabase/v3_features.sql",
  cursor_image_url: "supabase/v61_cursor_image.sql",
  cursor_image_size: "supabase/v62_cursor_image_size.sql",
  cursor_hotspot_x: "supabase/v107_cursor_hotspot.sql",
  cursor_hotspot_y: "supabase/v107_cursor_hotspot.sql",
  profile_favicon_url: "supabase/v79_profile_tab_branding.sql",
  tab_title_animation: "supabase/v79_profile_tab_branding.sql",
  bio_color: "supabase/v47_bio_customization.sql",
  bio_font_family: "supabase/v47_bio_customization.sql",
  bio_font_size: "supabase/v47_bio_customization.sql",
  bio_font_weight: "supabase/v47_bio_customization.sql",
  bio_italic: "supabase/v47_bio_customization.sql",
  bio_glow: "supabase/v47_bio_customization.sql",
  bio_letter_spacing: "supabase/v47_bio_customization.sql",
  username_effect: "supabase/v3_features.sql",
  show_view_count: "supabase/v3_features.sql",
  show_join_date: "supabase/v3_features.sql",
  profile_status: "supabase/v3_features.sql",
  featured_link_id: "supabase/v3_features.sql",
};

/** Which migration file(s) to run for missing columns */
export function getMigrationFilesForMissing(missing: string[]): string[] {
  const files = new Set<string>();

  if (missing.includes("profile_id") || missing.length > 20) {
    files.add("supabase/profiles.sql");
    files.add("supabase/links.sql");
    files.add("supabase/v2_features.sql");
  }

  for (const col of missing) {
    const file = COLUMN_MIGRATIONS[col];
    if (file) files.add(file);
  }

  if (files.size === 0) {
    return [
      "supabase/v3_features.sql",
      "supabase/v4_music_title.sql",
      "supabase/v11_ui_colors.sql",
      "supabase/v5_badges.sql",
      "supabase/v7_badge_customization.sql",
      "supabase/v10_links_parallax.sql",
      "supabase/v14_links_style.sql",
      "supabase/v12_v2_expansion.sql",
      "supabase/v13_show_follow_counts.sql",
      "supabase/v15_show_activity.sql",
      "supabase/v17_content_alignment.sql",
    ];
  }

  const ordered = [
    "supabase/profiles.sql",
    "supabase/links.sql",
    "supabase/v2_features.sql",
    "supabase/v3_features.sql",
    "supabase/v4_music_title.sql",
    "supabase/v11_ui_colors.sql",
    "supabase/v5_badges.sql",
    "supabase/v7_badge_customization.sql",
    "supabase/v10_links_parallax.sql",
    "supabase/v14_links_style.sql",
    "supabase/v12_v2_expansion.sql",
    "supabase/v13_show_follow_counts.sql",
    "supabase/v15_show_activity.sql",
    "supabase/v17_content_alignment.sql",
    "supabase/v24_enter_gate.sql",
    "supabase/v40_enter_gate_customization.sql",
  ];

  return ordered.filter((file) => files.has(file));
}

export function buildSchemaValidationMessage(missing: string[]): string {
  const files = getMigrationFilesForMissing(missing);
  const fileList = files.join(", then ");
  return `Missing profile_settings columns: ${missing.join(", ")}. Run ${fileList} in the Supabase SQL Editor, then restart the dev server.`;
}

export function isSchemaCacheError(message: string) {
  return /could not find the '([^']+)' column/i.test(message) ||
    /schema cache/i.test(message) ||
    /column profile_settings\.\w+ does not exist/i.test(message);
}

export function parseMissingColumn(message: string): string | null {
  const cacheMatch = message.match(/could not find the '([^']+)' column/i);
  if (cacheMatch?.[1]) return cacheMatch[1];

  const pgMatch = message.match(/column profile_settings\.(\w+) does not exist/i);
  return pgMatch?.[1] ?? null;
}

export function formatSchemaError(errorMessage: string): string {
  const missing = parseMissingColumn(errorMessage);
  if (missing) {
    return `Database schema is missing column "${missing}". ${SCHEMA_MIGRATION_HINT}`;
  }
  if (isSchemaCacheError(errorMessage)) {
    return `Database schema is out of date. ${SCHEMA_MIGRATION_HINT}`;
  }
  if (errorMessage.includes("profile_settings_layout_check")) {
    return "This layout is not enabled in the database yet. Run supabase/v99_premium_animated_layouts.sql in the Supabase SQL Editor, then save again.";
  }
  return errorMessage;
}
