...
export async function sendWelcomeEmail(input: {
  ...
  const subject = "Welcome to BioForge";

  const html = renderEmailLayout({
    preheader: "Your BioForge profile is ready.",
    bodyHtml: `
      <h1 style="margin:0 0 12px;font-size:22px;color:#ffffff;">Welcome, ${escapeHtml(name)}!</h1>
      <p style="margin:0;color:#a3a3a3;">Thanks for joining BioForge. Your profile is ready — customize your page, add links, and share it with the world.</p>
      ${emailButton(dashboardUrl, "Go to dashboard")}
    `,
  });

  const text = `Welcome to BioForge, ${name}!\n\nYour profile is ready. Open your dashboard: ${dashboardUrl}`;
...
export async function sendGuestbookNotificationEmail(input: {
  ...
  const html = renderEmailLayout({
    ...
    bodyHtml: `
      ...
      <blockquote style="margin:0;padding:12px 16px;border-left:3px solid #00e5cc;background:#0a0a0a;border-radius:0 8px 8px 0;color:#d4d4d4;">${escapeHtml(input.messagePreview)}</blockquote>
...
export async function sendBadgeNotificationEmail(input: {
  ...
  const html = renderEmailLayout({
    ...
    bodyHtml: `
      ...
      <p style="margin:0;color:#a3a3a3;">Congratulations! You earned the <strong style="color:#00e5cc;">${escapeHtml(input.badgeName)}</strong> badge.</p>
      ${description}
...
export async function sendPasswordResetEmail(input: {
  ...
  const subject = "Reset your BioForge password";

  const html = renderEmailLayout({
    preheader: "Use this link to reset your BioForge password.",
    bodyHtml: `
      <h1 style="margin:0 0 12px;font-size:20px;color:#ffffff;">Reset your password</h1>
      <p style="margin:0;color:#a3a3a3;">We received a request to reset your BioForge password. Click the button below to choose a new one. This link expires soon.</p>
  ...
  const text = `Reset your BioForge password:\n\n${input.resetUrl}\n\nIf you did not request this, ignore this email.`;
